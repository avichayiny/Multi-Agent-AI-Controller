...
# List of actions that can be performed by a robot.
...
class Game:
    ...
    def __init__(self, problem, debug=False):
        ...
        self._steps = 0
        self._reward = 0
        self._last_gained_reward = 0
        
        #self._robot_chosen_action_prob = problem["robot_chosen_action_prob"] # success rate.
        def seccess_rate(robot_id):
            caller = inspect.stack()[1]  # to check who called this method!
            if caller.function != "submit_next_action":
                raise ValueError("Nope! In RL, the transition function probabilities are hidden!")
            seccess_rate_dict = problem["robot_chosen_action_prob"]
            return seccess_rate_dict[robot_id]
        self._get_robot_seccess_rate = seccess_rate

        #self._plants_reward = problem['plants_reward'] # distribution over the plan rewards.
        def plant_reward_probabilies(plant_id):
            caller = inspect.stack()[1]  # to check who called this method!
            if caller.function != "submit_next_action":
                raise ValueError("Nope!, In RL reward distribution is hidden!")
            plants_reward_dict = problem['plants_reward']
            return plants_reward_dict[plant_id]
        self._get_plant_reward_probabilies = plant_reward_probabilies

        problem["plant_max_reward"] = {}
        for plant in problem["plants_reward"]:
            problem["plant_max_reward"][plant] = max(problem["plants_reward"][plant])
        self._plant_max_reward = problem["plant_max_reward"]

        self._problem = copy.deepcopy(problem)
        self._problem.pop("robot_chosen_action_prob")
        self._problem.pop("plants_reward")
    ...
    def get_plants_max_reward(self):
        return self._plant_max_reward
    ...
    def submit_next_action(self, chosen_action):
        ...
        p_success = self._get_robot_seccess_rate(robot_id)
        ...
        if success:
            ...
            # ---------- POUR action ----------
                ...
                reward_options = self._get_plant_reward_probabilies(robot_pos)
        ...
        # TODO check the this works correctly.
        self._last_gained_reward = add_reward
