...
class Controller:
    def __init__(self, game: ext_plant.Game):
        ...
        self.plant_values = {}
        raw_rewards = self.model.get("plants_reward", {})
        for pos, reward_list in raw_rewards.items():
            if reward_list:
                self.plant_values[pos] = sum(reward_list) / len(reward_list)
            else:
    ...
    def choose_next_action(self, state):
        ...
        for action_name, rid in valid_actions:
            ...
            for prob, next_state, reward in transitions:
                v_next = self.maximize_value(next_state, SEARCH_DEPTH - 1, real_steps_left - 1, action_name, rid)
                adjusted_reward = reward * 3 
    ...
    def maximize_value(self, state, depth, current_horizon, action, robot_id):
        ...
        if total_need == 0:
            res = self.model["goal_reward"] * 2 + (self.GAMMA * self.calculate_greedy_heuristic(self.initial_state, current_horizon - 1))
                ...
                if r_load > 0:
                    for p_pos, p_need in sim_plants.items():
                        ...
                        if real_cost <= sim_horizon:
                            ...
                            discounted_reward = amount * avg_val * (GAMMA ** arrival_time)
                    ...
                    if tap_pos:
                        ...
                        for p_pos, p_need in sim_plants.items():
                            ...
                            if total_real_cost <= sim_horizon:
                                ...
                                discounted_reward = needed_total * avg_val * (GAMMA ** arrival_time)
